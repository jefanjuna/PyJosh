...
class FeedforwardNeuralNetwork:
    ...
    def _get_config(self, configuration_file):
        self.key, subkey1, subkey2 = random.split(self.key, num=3)
        sample_config = {
            "layers": [
                {
                    "neurons": int(random.randint(subkey1, shape=(), minval=1, maxval=11, dtype=jnp.int32))
                }
            ]
        }
        del subkey1

        number_of_activations = len(self.supported_activation_functions)
        for i in range(random.randint(subkey2, shape=(), minval=1, maxval=11, dtype=jnp.int32)):
            if i == 0:
                del subkey2
            self.key, subkey1, subkey2 = random.split(self.key, num=3)
            index = random.randint(subkey1, shape=(), minval=0, maxval=number_of_activations, dtype=jnp.int32)
            del subkey1
            sample_config["layers"].append(
                {
                    "neurons": int(random.randint(subkey2, shape=(), minval=1, maxval=11, dtype=jnp.int32)),
                    "activation": self.supported_activation_functions[index]
